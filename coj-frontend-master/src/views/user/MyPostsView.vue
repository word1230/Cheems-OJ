<template>
  <div class="my-list-container">
    <div class="page-header">
      <h1 class="page-title">我的讨论</h1>
    </div>
    <a-spin :loading="loading" style="width:100%">
      <div class="list-wrapper">
        <a-empty v-if="!loading && list.length === 0" description="暂无讨论" style="padding: 60px 0" />
        <div
          v-for="post in list"
          :key="post.id"
          class="post-item"
...
onMounted(async () => {
  loading.value = true;
  const res = await PostControllerService.listMyPostVoByPageUsingPost({ pageSize: 20, current: 1 });
